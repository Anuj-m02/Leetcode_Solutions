...
class Solution:
    def wiggleMaxLength(self, nums: list[int]) -> int:
        ...
        # direction: 1 means we need an UPward step next, 0 means DOWNward
        @lru_cache(maxsize=None)
        def dp(indx, is_up):
            max_len = 0
            for i in range(indx + 1, n):
                if is_up and nums[i] > nums[indx]:
                    max_len = max(max_len, 1 + dp(i, False))
                elif not is_up and nums[i] < nums[indx]:
                    max_len = max(max_len, 1 + dp(i, True))
            return max_len

        # Try starting the sequence from index 0 either going UP next or DOWN next
        return 1 + max(dp(0, True), dp(0, False))
